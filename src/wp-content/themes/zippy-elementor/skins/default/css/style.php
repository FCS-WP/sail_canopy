...
if ( ! function_exists( 'jigsaw_customizer_get_css' ) ) {

	function jigsaw_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = jigsaw_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.jigsaw_tabs .jigsaw_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'jigsaw_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = jigsaw_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'jigsaw_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( jigsaw_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = jigsaw_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'jigsaw_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'jigsaw_filter_prepare_css', $css_str, $remove_spaces );
	}
}
