...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			jigsaw_create_observer( 'jigsaw-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				jigsaw_gutenberg_editor_init();
			} );
		} else {
		 	jigsaw_gutenberg_editor_init();
		}
	...
	function jigsaw_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'jigsaw_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'jigsaw_inited' ) ) {
	...
	// Init on page load
	function jigsaw_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = jigsaw_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + JIGSAW_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + JIGSAW_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in JIGSAW_STORAGE['override_classes'] ) {
				$obj.addClass( JIGSAW_STORAGE['override_classes'][i].replace( '%s', JIGSAW_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( jigsaw_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && JIGSAW_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'jigsaw_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			jigsaw_remove_observer( 'jigsaw-check-editor-styles-wrapper' );
			jigsaw_create_observer( 'jigsaw-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'jigsaw_inited' );
	}
} );
