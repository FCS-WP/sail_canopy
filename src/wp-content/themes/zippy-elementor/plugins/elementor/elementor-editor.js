...
jQuery( document ).ready(
	function() {
		...
		jigsaw_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="jigsaw_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.jigsaw_post_editor' );
						if ( link.length > 0 ) {
							jigsaw_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.jigsaw_post_editor', function(e) {
						jigsaw_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('jigsaw_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function jigsaw_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
