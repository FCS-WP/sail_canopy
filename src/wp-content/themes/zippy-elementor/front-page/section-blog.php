<div class="front_page_section front_page_section_blog<?php
	$jigsaw_scheme = jigsaw_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $jigsaw_scheme ) && ! jigsaw_is_inherit( $jigsaw_scheme ) ) {
		echo ' scheme_' . esc_attr( $jigsaw_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( jigsaw_get_theme_option( 'front_page_blog_paddings' ) );
	if ( jigsaw_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$jigsaw_css      = '';
		$jigsaw_bg_image = jigsaw_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$jigsaw_anchor_icon = jigsaw_get_theme_option( 'front_page_blog_anchor_icon' );
	$jigsaw_anchor_text = jigsaw_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( jigsaw_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$jigsaw_css      = '';
			$jigsaw_bg_mask  = jigsaw_get_theme_option( 'front_page_blog_bg_mask' );
			$jigsaw_bg_color_type = jigsaw_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $jigsaw_bg_color_type ) {
				$jigsaw_bg_color = jigsaw_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$jigsaw_caption = jigsaw_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $jigsaw_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $jigsaw_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $jigsaw_caption, 'jigsaw_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$jigsaw_description = jigsaw_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $jigsaw_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $jigsaw_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $jigsaw_description ), 'jigsaw_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! jigsaw_exists_trx_addons() ) {
						...
						jigsaw_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
